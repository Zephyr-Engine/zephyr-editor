...
fn createDockTree(dock: *ui.DockSpace, nodes: EditorNodes) !EditorDockRefs {
    ...
    try dock.moveWindowToLeaf(viewport_window, dock.rootLeaf());
    ...
    try dock.moveWindowToLeaf(inspector_window, right.new_leaf);
    ...
    try dock.moveWindowToLeaf(console_window, bottom.new_leaf);
    ...
    try dock.moveWindowToLeaf(scene_window, left.new_leaf);
